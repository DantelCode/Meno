# Meno
A comprehensive web-based life organization application designed to help professionals and individuals streamline their daily routines.


### ✅ The Solution - MENO Offers:
📊 **Comprehensive Dashboard** - Get an at-a-glance overview of your day with progress tracking and analytics
<br>
📅 **Interactive Calendar System** - Visualize all your commitments with intuitive month navigation and smart event indicators
<br>
🍽️ **Integrated Meal Planner** - Plan your meals with an easy-to-use meal management system
<br>
🛒 **Smart Shopping List** - Organize and manage your shopping efficiently
<br>
🌍 **Google Calendar Integration** - Automatically sync holidays and events for better coordination
<br>
🎨 **Professional UI/UX** - Beautiful, responsive design with dark mode support for comfortable all-day usage
<br>
🔒 **Data Security** - Secure local storage with XSS protection and proper data management


### 🛠️ Built With:
- Node.js & Express Backend
- Vanilla JavaScript Frontend
- EJS Templating
- Advanced CSS with Theme Variables
- Google Calendar API Integration
- EmailJS for User Feedback


### Live Site: https://meno-orpin.vercel.app/


### Getting started:
- Install: npm install
- Uncomment: api/server.js(line 28-36)
- Run: node api/server.js
- Open: http://localhost:3000
- Note: Fetch Google Events and Send Feedback will require Google Api Key and EmailJS credentials to function.


### Feature Implementation:
- User accounts & DB persistence (e.g., MongoDB) to sync data across devices
- Reminders/notifications (push or email) for upcoming events and shopping
- Multiple calendar integrations (Google, Outlook) and color tags
- Offline support and background sync with Service Worker


This project demonstrates my commitment to building practical, user-centered solutions that improve daily productivity.
I'm open to feedback and continued improvements. Feel free to reach out if you'd like to discuss the project or explore collaboration opportunities. And give this repo a star to keep encouraging me.

## Screenshots
![Meno Screenshot 1](https://github.com/DantelCode/Meno/blob/main/api/screenshots/meno_logo.PNG)
![Meno Screenshot 2](https://github.com/DantelCode/Meno/blob/main/api/screenshots/1.PNG)
![Meno Screenshot 3](https://github.com/DantelCode/Meno/blob/main/api/screenshots/2.PNG)
![Meno Screenshot 4](https://github.com/DantelCode/Meno/blob/main/api/screenshots/3.png)
![Meno Screenshot 5](https://github.com/DantelCode/Meno/blob/main/api/screenshots/4.png)
![Meno Screenshot 6](https://github.com/DantelCode/Meno/blob/main/api/screenshots/5.png)
![Meno Screenshot 7](https://github.com/DantelCode/Meno/blob/main/api/screenshots/6.png)
![Meno Screenshot 8](https://github.com/DantelCode/Meno/blob/main/api/screenshots/7.png)

